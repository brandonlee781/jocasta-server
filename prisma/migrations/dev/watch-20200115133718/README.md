# Migration `watch-20200115133718`

This migration has been generated by Brandon Lee at 1/15/2020, 1:37:19 PM.
You can check out the [state of the schema](./schema.prisma) after the migration.

## Database Steps

```sql
ALTER TABLE "jocasta_schema"."Armor" DROP COLUMN "condition",
ADD COLUMN "condition" text    ,
DROP COLUMN "description",
ADD COLUMN "description" text    ;

ALTER TABLE "jocasta_schema"."Character" DROP COLUMN "background",
ADD COLUMN "background" text    ,
DROP COLUMN "createdAt",
ADD COLUMN "createdAt" timestamp(3)  NOT NULL DEFAULT '1970-01-01 00:00:00' ,
DROP COLUMN "credits",
ADD COLUMN "credits" integer    ,
DROP COLUMN "description",
ADD COLUMN "description" text    ,
DROP COLUMN "imageUrl",
ADD COLUMN "imageUrl" text    ,
DROP COLUMN "injuries",
ADD COLUMN "injuries" text    ,
DROP COLUMN "motivation",
ADD COLUMN "motivation" text    ,
DROP COLUMN "notes",
ADD COLUMN "notes" text    ,
DROP COLUMN "resources",
ADD COLUMN "resources" text    ,
DROP COLUMN "updatedAt",
ADD COLUMN "updatedAt" timestamp(3)  NOT NULL DEFAULT '1970-01-01 00:00:00' ;

ALTER TABLE "jocasta_schema"."DefenseAttribute" DROP COLUMN "derivedAttributes",
ADD COLUMN "derivedAttribute" text    REFERENCES "jocasta_schema"."DerivedAttributes"("id") ON DELETE SET NULL;

ALTER TABLE "jocasta_schema"."DerivedAttributes" DROP COLUMN "defense";

ALTER TABLE "jocasta_schema"."Gear" DROP COLUMN "description",
ADD COLUMN "description" text    ;

ALTER TABLE "jocasta_schema"."Talent" DROP COLUMN "description",
ADD COLUMN "description" text    ,
DROP COLUMN "page",
ADD COLUMN "page" text    ;

ALTER TABLE "jocasta_schema"."TalentTreeNode" DROP COLUMN "connections",
ADD COLUMN "connections" integer []   ;

ALTER TABLE "jocasta_schema"."User" DROP COLUMN "name",
ADD COLUMN "name" text    ;

ALTER TABLE "jocasta_schema"."Weapon" DROP COLUMN "condition",
ADD COLUMN "condition" text    ,
DROP COLUMN "description",
ADD COLUMN "description" text    ,
DROP COLUMN "make",
ADD COLUMN "make" text    ;

CREATE UNIQUE INDEX "DefenseAttribute_derivedAttribute" ON "jocasta_schema"."DefenseAttribute"("derivedAttribute")

CREATE  INDEX "User.email_name" ON "jocasta_schema"."User"("email","name")
```

## Changes

```diff
diff --git schema.prisma schema.prisma
migration watch-20200115133303..watch-20200115133718
--- datamodel.dml
+++ datamodel.dml
@@ -3,9 +3,9 @@
 }
 datasource db {
   provider = "postgresql"
-  url = "***"
+  url      = "postgresql://brandonlee:Srf32pstgrs@localhost:5432/jocasta?schema=jocasta_schema"
 }
 enum GearCondition {
   NONE
@@ -84,12 +84,12 @@
   strainThreshold DerivedAttributes? @relation(name:"StrainThreshold", onDelete:CASCADE)
 }
 model DefenseAttribute {
-  id                String            @default(cuid()) @id
-  ranged            Int               @default(0)
-  melee             Int               @default(0)
-  derivedAttributes DefenseAttribute? @relation(onDelete: CASCADE)
+  id                String              @default(cuid()) @id
+  ranged            Int                 @default(0)
+  melee             Int                 @default(0)
+  derivedAttribute  DerivedAttributes?  @relation(onDelete: CASCADE)
 }
 model Duty {
   id            String  @default(cuid()) @id
@@ -115,12 +115,12 @@
   presence  Int     @default(0)
 }
 model DerivedAttributes {
-  id      String    @default(cuid()) @id
+  id      String            @default(cuid()) @id
   soak    Int
-  wounds  Threshold @relation("WoundsThreshold")
-  strain  Threshold @relation("StrainThreshold")
+  wounds  Threshold         @relation("WoundsThreshold")
+  strain  Threshold         @relation("StrainThreshold")
   defense DefenseAttribute
   character Character?
 }
```


